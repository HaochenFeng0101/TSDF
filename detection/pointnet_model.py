...
class STN3d(nn.Module):
    ...
    def forward(self, x):
        ...
        identity = torch.eye(3, dtype=x.dtype, device=x.device).reshape(1, 9)
...
class STNkd(nn.Module):
    ...
    def forward(self, x):
        ...
        identity = torch.eye(self.k, dtype=x.dtype, device=x.device).reshape(1, -1)
        x = x + identity.repeat(batch_size, 1)
...
def feature_transform_regularizer(trans):
    if trans is None:
        return 0.0
    dim = trans.size(1)
    identity = torch.eye(dim, device=trans.device, dtype=trans.dtype).unsqueeze(0)
