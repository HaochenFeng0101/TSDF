...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Inspect one PointNeXt prediction on ScanObjectNN/ModelNet40 or an external point cloud."
    )
    parser.add_argument("--checkpoint", default=str(TSDF_ROOT / "model" / "pointnext" / "pointnext_best.pth"))
    parser.add_argument("--labels", default=str(TSDF_ROOT / "model" / "pointnext" / "labels.txt"))
    ...
    model = PointNeXtSmallCls(num_classes=len(labels), input_channels=3, dropout=dropout).to(args.device)
