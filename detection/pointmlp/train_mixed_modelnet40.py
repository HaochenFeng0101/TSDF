...
"""
python detection/pointmlp/train_mixed_modelnet40.py
...
def compute_sampled_mild_size(mild_dataset_size, mild_ratio_denominator):
    mild_ratio_denominator = int(mild_ratio_denominator)
    if mild_ratio_denominator <= 0:
        raise ValueError("--mild-ratio-denominator must be a positive integer.")
    if mild_dataset_size <= 0:
        return 0
    if mild_ratio_denominator == 1:
        return mild_dataset_size
    return max(1, mild_dataset_size // mild_ratio_denominator)
...
def modelnet40_root_looks_like_dir(root):
    try:
        build_modelnet40_dir_splits(root)
        return True
    except Exception:
        return False
...
def build_modelnet40_dir_splits(data_root):
    data_root = Path(data_root)
    if not data_root.exists():
        raise FileNotFoundError(f"Dataset root does not exist: {data_root}")

    labels = read_modelnet40_labels(data_root)
...
def get_mixed_modelnet40_dataloaders(
    ...
    if modelnet40_root_looks_like_h5(mild_root):
        mild_labels, train_dataset_mild_full = load_modelnet40_h5_dataset(
            root=mild_root,
            split="train",
            num_points=num_points,
            augment=True,
    ...
    elif modelnet40_root_looks_like_dir(mild_root):
        mild_labels, mild_train_samples, _ = build_modelnet40_dir_splits(mild_root)
        train_dataset_mild_full = PointCloudClassificationDataset(
            mild_train_samples,
            num_points,
            mild_labels,
            augment=True,
        ...
        raise FileNotFoundError(
            f"Could not find a usable processed ModelNet40 dataset under {Path(mild_root)}"
        )
    ...
    target_mild_size = compute_sampled_mild_size(
        mild_dataset_size=len(train_dataset_mild_full),
        mild_ratio_denominator=mild_ratio_denominator,
    )
    train_dataset_mild = sample_mild_dataset(
        train_dataset_mild_full,
        target_size=target_mild_size,
        seed=seed + 23,
    )
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Train a PointMLP classifier on original + mildly processed ModelNet40."
    )
    parser.add_argument(
        "--modelnet40-root",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Sample 1/N of the processed ModelNet40 train split. 5 means randomly use about 1/5 of the mild dataset.",
    )
...
def main(argv=None):
    ...
    print(
        ...
        f"num_classes={len(labels)} | sampled_mild_fraction=1/{args.mild_ratio_denominator}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ckpt = {
            ...
            "val_acc": val_acc,
            "task": "classification",
            "dataset": "ModelNet40(raw+mild)",
            "dataset_type": "modelnet40",
            "modelnet40_root": str(Path(args.modelnet40_root).resolve()),
            "modelnet40_mild_root": str(Path(args.modelnet40_mild_root).resolve()),
