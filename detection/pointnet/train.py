...
from TSDF.detection.pointnet.pointnet_cls import PointNetCls, feature_transform_regularizer
...
def evaluate(
    ...
    with torch.no_grad():
        for points, labels in iterator:
            ...
            reg = feature_transform_weight * feature_transform_regularizer(trans_feat)
            loss = ce + reg
...
def main():
    parser = argparse.ArgumentParser(
        description="Train a PointNet classifier on ScanObjectNN or ModelNet40 with optional extra/mild data."
    )
    parser.add_argument(
        "--dataset-type",
    ...
    parser.add_argument("--output-dir", default=str(TSDF_ROOT / "model" / "pointnet"))
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--use-wandb", action="store_true")
    parser.add_argument("--wandb-project", default="TSDF-PointNet")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for points, labels_batch in iterator:
            ...
            with torch.amp.autocast(device_type=amp_device_type, enabled=use_amp):
                ...
                reg_loss = args.feature_transform_weight * feature_transform_regularizer(trans_feat)
